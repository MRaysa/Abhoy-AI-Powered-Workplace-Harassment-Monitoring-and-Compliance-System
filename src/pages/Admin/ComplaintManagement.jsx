...
const ComplaintManagement = () => {
  ...
  const fetchComplaints = async () => {
    setLoading(true);
    try {
      let url = `https://abhoy-server.vercel.app/api/complaints?page=${page}&limit=10`;
  ...
  const updateStatus = async (anonymousId, newStatus) => {
    try {
      ...
      const response = await fetch(
        `https://abhoy-server.vercel.app/api/complaints/${anonymousId}/status`,
        {
          method: "PATCH",
          headers: {
  ...
  const approveForForum = async (anonymousId, approved) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await fetch(
          `https://abhoy-server.vercel.app/api/complaints/${anonymousId}/approve-forum`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
  ...
  const viewDetails = async (anonymousId) => {
    try {
      const response = await fetch(
        `https://abhoy-server.vercel.app/api/complaints/${anonymousId}`
      );
      const result = await response.json();
